  ...
  while true do
    ...
    if not chatInputActive then
      if IsControlPressed(0, isRDR and `INPUT_MP_TEXT_CHAT_ALL` or 245) --[[ INPUT_MP_TEXT_CHAT_ALL ]] then
        ...
        LocalPlayer.state:set('isTyping', true, true)
    ...
    if not chatInputActive then
      LocalPlayer.state:set('isTyping', false, true)
    end
...
Citizen.CreateThread(function()
    ...
    local playerCoords = GetEntityCoords(playerPed)
		...
		for _, i in ipairs(GetActivePlayers()) do
			...
			local plyState = Player(serverid).state
      local distance = #(playerCoords - targetCoords)

			if targetPed ~= playerPed then
        if distance < 10 then
			    if plyState.isTyping and not plyState.isDead then
				    DrawText3D(targetCoords.x, targetCoords.y, targetCoords.z, '[...]', 202, 131, 67)
			    end
			  end
		  end
    end
...
function DrawText3D(x, y, z, text, r, g, b)
	local onScreen, _x, _y = GetScreenCoordFromWorldCoord(x, y, z+1.2)
